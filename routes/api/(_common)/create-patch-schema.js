 ...
 * @example
 */
const createPatchSchema = (table, { from = [] } = {}) => {
	...
	finalSchema = finalSchema.partial().strict();
		...
		.superRefine((data, context) => {
			if (Object.keys(data).length === 0) {
				context.addIssue({
					...
					message: "At least one field should be set"
				});
			}
...
export default createPatchSchema;
